...
describe('GET /api/rutas-pasadas-articulos — error paths', () => {
  it('returns 400 when rutaPasadaId query param is invalid', async () => {
    const res = await request(app)
      .get('/api/rutas-pasadas-articulos?rutaPasadaId=notanumber')
      .set('Authorization', `Bearer ${adminToken()}`);
...
describe('GET /api/rutas-pasadas-articulos/:id', () => {
  it('returns a single pivot record', async () => {
    const pivot = { id: 1, articulo: { id: 2, nombre: 'Harina' } };
    mockEm.findOne.mockResolvedValue(pivot);

    const res = await request(app)
      .get('/api/rutas-pasadas-articulos/1')
      .set('Authorization', `Bearer ${adminToken()}`);
  ...
  it('returns 404 when record not found', async () => {
    ...
    const res = await request(app)
      .get('/api/rutas-pasadas-articulos/999')
      .set('Authorization', `Bearer ${adminToken()}`);
...
describe('POST /api/rutas-pasadas-articulos — error paths', () => {
...
describe('DELETE /api/rutas-pasadas-articulos/:id — error paths', () => {
  it('returns 500 on unexpected service error', async () => {
    mockEm.findOne.mockRejectedValue(new Error('DB down'));

    const res = await request(app)
      .delete('/api/rutas-pasadas-articulos/1')
      .set('Authorization', `Bearer ${adminToken()}`);
